...
class ExecutiveDashboard:
    ...
    def launch_credit_memo_dashboard(self, e):
        ...
        try:
            ...
            if not os.path.exists(credit_memo_file):
                self.show_snackbar_simple("❌ Archivo cm.py no encontrado", "#ef4444")
            ...
            self.show_snackbar_simple("✅ Credit Memo Dashboard iniciado", "#15803d")
    ...
    def launch_kiteo_dashboard(self, e):
        ...
        try:
            # Mostrar mensaje de carga
            self.show_snackbar_simple("🚀 Iniciando Grupos/Kiteo Dashboard...", "#7c3aed")
            ...
            # Mostrar confirmación
            self.show_snackbar_simple(f"✅ Grupos/Kiteo Dashboard iniciado", "#15803d")
    ...
    def show_dashboard(self):
        ...
        def create_nav_item(icon, label, index, permission):
            ...
            return ft.Container(
                ...
                height=60,
                on_click=lambda _, idx=index: change_tab(idx),
                animate=ft.Animation(200, ft.AnimationCurve.EASE_IN_OUT),
            )
        ...
        sidebar = ft.Container(
            width=250,
            bgcolor="#2e2e2e",  # Gris oscuro personalizado
            content=ft.Column([
                # Logo/Header
                ft.Container(
                    content=ft.Column([
        ...
        def create_finance_content():
            if self.current_user['role'] != 'admin':
                return ft.Container(
                    content=ft.Column([
                        ft.Icon(ft.Icons.LOCK, size=80, color="#ef4444"),  # Rojo
        ...
        def create_users_management_content():
            ...
            def refresh_users():
                ...
                for user in users:
                    ...
                    admin_color = ft.Colors.YELLOW if user['role'] == 'admin' else self.theme_color
                    admin_bg_color = "#2e2e2e" if user['role'] == 'admin' else "#1a1a1a"
                    admin_border_color = ft.Colors.YELLOW if user['role'] == 'admin' else ft.Colors.TRANSPARENT
                    ...
                    user_card = ft.Container(
                        ...
                        padding=20,
                        bgcolor=admin_bg_color,
                        border=ft.border.all(2, admin_border_color),
                        border_radius=15,
            ...
            def save_user_changes(e):
                ...
                try:
                    ...
                    self.show_snackbar_simple("✅ Usuario actualizado correctamente", "#15803d")
                    edit_section.visible = False
                    selected_user[0] = None
                    refresh_users()
                    
                except Exception as ex:
                    self.show_snackbar_simple(f"❌ Error: {str(ex)}", "#ef4444")
            ...
            def delete_user(employee_id):
                if self.db.delete_user(employee_id):
                    self.show_snackbar_simple("✅ Usuario eliminado", "#15803d")
                    refresh_users()
                else:
                    self.show_snackbar_simple("❌ No se puede eliminar el último administrador", "#ef4444")
            
            def create_user(e):
                try:
                    if not all([new_employee_id.value, new_first_name.value, new_last_name.value]):
                        self.show_snackbar_simple("⚠️ Complete todos los campos", "#f97316")
                        return
                    ...
                    self.show_snackbar_simple(f"❌ {str(ex)}", "#ef4444")
            
            # Crear sección de edición
            edit_section.content = ft.Column([
                ft.Text("✏️ Editar Usuario", size=18, weight=ft.FontWeight.W_500, color="#3b82f6"),  # Azul
            ...
            return ft.Container(
                content=ft.Column([
                    ...
                    ft.Container(
                        content=ft.Column([
                            ft.Text("➕ Crear Nuevo Usuario", size=20, weight=ft.FontWeight.W_500, color="#10b981"),  # Verde
                            ft.Container(height=15),
                        ...
                        padding=25,
                        bgcolor="#0f5132",  # Verde oscuro
                        border_radius=15,
                        border=ft.border.all(1, "#198754"),  # Verde medio
                    ),
                    ...
                    ft.Container(
                        ...
                        bgcolor="#1e3a8a",  # Azul oscuro
                        border_radius=15,
                        border=ft.border.all(1, "#3b82f6"),  # Azul medio
                    ),
            ...
            for i in range(12):
                ...
                bars.append(
                    ft.Container(
                        width=30,
                        height=height,
                        bgcolor=self.theme_color if i == 11 else "#3b82f6",  # Azul claro
    ...
    def on_generate_report(self, e):
        print(f"Generando reporte para {self.current_user['first_name']} {self.current_user['last_name']}..")
        self.show_snackbar_simple("📄 Reporte generado exitosamente", "#15803d")
