...
class InventoryDashboard:
    ...
    def load_data_from_db(self):
        ...
        try:
            ...
            # ← NUEVO: Cargar tabla de inventario OH
            query_oh = "SELECT ItemNo, OH FROM oh"
            self.df_oh = pd.read_sql_query(query_oh, conn)
            
            conn.close()
            print(f"✅ Datos cargados: {len(self.df_original)} registros totales")
            print(f"✅ Inventario OH: {len(self.df_oh)} registros de inventario")  # ← NUEVO
            return True
    ...
    def process_data(self):
        ...
        if self.df_original is None or self.df_costs is None or self.df_oh is None:  # ← MODIFICADO: agregamos or self.df_oh is None
            return False
        ...
        # ← NUEVO: Hacer cruce con tabla OH para obtener inventario disponible
        df = df.merge(self.df_oh, left_on='ItemNo', right_on='ItemNo', how='left')

        # ← NUEVO: Renombrar columna OH a QtyOh para mantener compatibilidad
        df['QtyOh'] = df['OH']
    ...
    def get_wo_analysis(self, wo_number=None):
        ...
        try:
            ...
            if wo_number:
                # MODIFICADO: Incluir cruce con tabla OH
                query = """
                SELECT pr.*, in92.STDCost, oh.OH as QtyOh_Real
                FROM pr561 pr 
                LEFT JOIN in92 ON pr.ItemNo = in92.ItemNo 
                LEFT JOIN oh ON pr.ItemNo = oh.ItemNo
                WHERE pr.WONo = ? 
                ...
                SELECT pr.WONo, pr.ItemNo, pr.QtyPending, pr.ReqQty, in92.STDCost,
                    pr.Project, pr.Entity, oh.OH as QtyOh_Real
                FROM pr561 pr 
                LEFT JOIN in92 ON pr.ItemNo = in92.ItemNo 
                LEFT JOIN oh ON pr.ItemNo = oh.ItemNo
            ...
            df_wo['QtyOh_Real'] = pd.to_numeric(df_wo['QtyOh_Real'], errors='coerce').fillna(0)
    ...
    def get_wo_clear_by_component_count(self):
        """Clasificar WO Clear por cantidad de componentes: 1, 2, y 3-5"""
        try:
            conn = sqlite3.connect(self.db_path)
            
            # Consultar WO Clear con conteo de componentes
            query = """
            SELECT WONo, COUNT(*) as Component_Count,
                SUM(Valor_Cubierto) as Total_Value,
                MAX(Project) as Project, 
                MAX(Entity) as Entity,
                GROUP_CONCAT(Component, ', ') as Components_List
            FROM WOClear 
            GROUP BY WONo 
            ORDER BY Component_Count, Total_Value DESC
            """
            
            df_wo_analysis = pd.read_sql_query(query, conn)
            conn.close()
            
            if len(df_wo_analysis) == 0:
                return {
                    'wo_1_component': pd.DataFrame(),
                    'wo_2_components': pd.DataFrame(),
                    'wo_3_to_5_components': pd.DataFrame(),
                    'wo_more_than_5': pd.DataFrame(),
                    'summary': {
                        'total_wo_clear': 0,
                        'wo_1_count': 0, 'wo_1_value': 0,
                        'wo_2_count': 0, 'wo_2_value': 0,
                        'wo_3_5_count': 0, 'wo_3_5_value': 0,
                        'wo_more_5_count': 0, 'wo_more_5_value': 0
                    }
                }
            
            # Clasificar por cantidad de componentes
            wo_1_component = df_wo_analysis[df_wo_analysis['Component_Count'] == 1].copy()
            wo_2_components = df_wo_analysis[df_wo_analysis['Component_Count'] == 2].copy()
            wo_3_to_5_components = df_wo_analysis[
                (df_wo_analysis['Component_Count'] >= 3) & 
                (df_wo_analysis['Component_Count'] <= 5)
            ].copy()
            wo_more_than_5 = df_wo_analysis[df_wo_analysis['Component_Count'] > 5].copy()
            
            # Agregar ranking dentro de cada categoría
            wo_1_component['Ranking'] = range(1, len(wo_1_component) + 1)
            wo_2_components['Ranking'] = range(1, len(wo_2_components) + 1)
            wo_3_to_5_components['Ranking'] = range(1, len(wo_3_to_5_components) + 1)
            wo_more_than_5['Ranking'] = range(1, len(wo_more_than_5) + 1)
            
            # Calcular métricas resumen
            summary = {
                'total_wo_clear': len(df_wo_analysis),
                'wo_1_count': len(wo_1_component),
                'wo_1_value': float(wo_1_component['Total_Value'].sum()) if len(wo_1_component) > 0 else 0,
                'wo_2_count': len(wo_2_components),
                'wo_2_value': float(wo_2_components['Total_Value'].sum()) if len(wo_2_components) > 0 else 0,
                'wo_3_5_count': len(wo_3_to_5_components),
                'wo_3_5_value': float(wo_3_to_5_components['Total_Value'].sum()) if len(wo_3_to_5_components) > 0 else 0,
                'wo_more_5_count': len(wo_more_than_5),
                'wo_more_5_value': float(wo_more_than_5['Total_Value'].sum()) if len(wo_more_than_5) > 0 else 0
            }
            
            return {
                'wo_1_component': wo_1_component,
                'wo_2_components': wo_2_components,
                'wo_3_to_5_components': wo_3_to_5_components,
                'wo_more_than_5': wo_more_than_5,
                'summary': summary
            }
            
        except Exception as e:
            print(f"Error en análisis por componentes: {e}")
            return {
                'wo_1_component': pd.DataFrame(),
                'wo_2_components': pd.DataFrame(),
                'wo_3_to_5_components': pd.DataFrame(),
...
def create_component_analysis_cards(component_data):
    """Crear tarjetas de análisis por cantidad de componentes"""
    summary = component_data['summary']
    
    return ft.Row([
        create_metric_card(
            "WO con 1 Componente",
            f"{summary['wo_1_count']:,}",
            f"Valor: ${summary['wo_1_value']:,.0f}",
            COLORS['success']
        ),
        create_metric_card(
            "WO con 2 Componentes", 
            f"{summary['wo_2_count']:,}",
            f"Valor: ${summary['wo_2_value']:,.0f}",
            COLORS['warning']
        ),
        create_metric_card(
            "WO con 3-5 Componentes",
            f"{summary['wo_3_5_count']:,}",
            f"Valor: ${summary['wo_3_5_value']:,.0f}",
            COLORS['accent']
        ),
        create_metric_card(
            "WO con +5 Componentes",
            f"{summary['wo_more_5_count']:,}",
            f"Valor: ${summary['wo_more_5_value']:,.0f}",
            COLORS['error']
        ),
    ], wrap=True, spacing=15)

def create_component_category_table(df_data, category_name, icon, color):
    """Crear tabla para una categoría específica de componentes"""
    ...
    data_table = ft.DataTable(
        columns=[
            ...
            ft.DataColumn(ft.Text("Components List", color=COLORS['text_primary'])),
        ],
        rows=[],
    ...
    for _, row in df_data.iterrows():
        ...
        if row['Ranking'] <= 3:
            ...
            ranking_color = color  # Resto en color de categoría
        
        # Truncar lista de componentes si es muy larga
        components_list = str(row['Components_List'])
        if len(components_list) > 50:
            components_list = components_list[:47] + "..."
        ...
        data_table.rows.append(
            ft.DataRow(
                cells=[
                    ...
                    ft.DataCell(ft.Text(f"{row['Component_Count']}", color=color, weight=ft.FontWeight.BOLD, selectable=True)),
                    ...
                    ft.DataCell(ft.Text(str(row['Entity']) if row['Entity'] else "-", color=COLORS['text_secondary'], selectable=True)),
                    ft.DataCell(ft.Text(components_list, color=COLORS['accent'], selectable=True)),
                ]
            )
        )
    ...
    if len(df_data) > 0:
        avg_value = df_data['Total_Value'].mean()
        max_value = df_data['Total_Value'].max()
        total_value = df_data['Total_Value'].sum()
        
        stats_text = f"Promedio: ${avg_value:,.0f} | Máximo: ${max_value:,.0f} | Total: ${total_value:,.0f}"
    else:
        stats_text = "No hay datos para esta categoría"
    
    return ft.Container(
        content=ft.Column([
            ft.Row([
                ft.Text(f"{icon} {category_name}", 
                       size=18, weight=ft.FontWeight.BOLD, color=color),
                ft.Text(f"({len(df_data)} WOs)", 
                       size=14, color=COLORS['text_secondary'])
            ]),
            ft.Text(stats_text, size=12, color=COLORS['text_secondary']),
            ft.Container(
                content=ft.Column([
                    data_table
                ...
                height=300,
                bgcolor=COLORS['primary'],
            ) if len(df_data) > 0 else ft.Container(
                content=ft.Text("No hay Work Orders en esta categoría", 
                               size=14, color=COLORS['text_secondary']),
...
def create_wo_clear_tab(dashboard):
    ...
    # NUEVO: Referencias para inicialización
    init_container = ft.Ref[ft.Container]()
    
    def initialize_wo_clear_tab(e):
        """Inicializar la pestaña WO Clear con datos"""
        try:
            init_container.current.visible = False
            init_container.current.update()
            
            refresh_filters()
            load_wo_clear_data()
            
        except Exception as error:
            print(f"Error inicializando WO Clear: {error}")
            init_container.current.visible = True
            init_container.current.update()
    ...
    component_analysis_container = ft.Ref[ft.Container]()
    ...
    def search_wo_materials(e):
        ...
        try:
            ...
            # MODIFICADO: Consultar materiales con inventario desde tabla OH
            query = """
            SELECT pr.ItemNo as Component, pr.ReqDate, 
                CAST(COALESCE(oh.OH, 0) AS REAL) as QtyOh,       # ← CAMBIO: oh.OH en lugar de pr.QtyOh
            ...
            FROM pr561 pr 
            LEFT JOIN in92 ON pr.ItemNo = in92.ItemNo 
            LEFT JOIN oh ON pr.ItemNo = oh.ItemNo               # ← NUEVO: Agregar JOIN con tabla OH
            WHERE pr.WONo = ?
            ORDER BY pr.ReqDate, pr.ItemNo
            """
    ...
    def load_wo_clear_data():
        ...
        try:
            ...
            if len(df_wo_clear) == 0:
                ...
                wo_materials_container.current.visible = False
                wo_inquiry_container.current.visible = False
                # NUEVO: Limpiar contenedor de análisis por componentes
                component_analysis_container.current.visible = False
            ...
            # NUEVO: Obtener análisis por componentes
            component_analysis = dashboard.get_wo_clear_by_component_count()
            ...
            wo_clear_metrics.current.controls = [
                ...
                # NUEVO: Métricas por cantidad de componentes
                ft.Text("📊 Análisis por Cantidad de Componentes", 
                        size=18, weight=ft.FontWeight.BOLD, color=COLORS['text_primary']),
                create_component_analysis_cards(component_analysis)
            ]
            
            # NUEVO: Actualizar contenedor de análisis por componentes
            update_component_analysis_display(component_analysis)
...
🎯 WOs críticas: {', '.join(pareto_data.head(3)['WONo'].astype(str))}"""
            ...
            component_analysis_container.current.update()
    ...
    def update_component_analysis_display(component_data):
        """Actualizar el display del análisis por componentes"""
        
        # Limpiar contenido anterior
        component_analysis_container.current.content.controls.clear()
        
        if component_data['summary']['total_wo_clear'] == 0:
            component_analysis_container.current.visible = False
            return
        
        # Agregar título
        component_analysis_container.current.content.controls.append(
            ft.Text("🔍 Análisis Detallado por Cantidad de Componentes", 
                   size=20, weight=ft.FontWeight.BOLD, color=COLORS['text_primary'])
        )
        
        component_analysis_container.current.content.controls.append(
            ft.Text("Work Orders clasificadas según la complejidad de componentes", 
                   size=12, color=COLORS['text_secondary'])
        )
        
        # Crear pestañas para cada categoría
        component_tabs = ft.Tabs(
            selected_index=0,
            animation_duration=300,
            tabs=[
                ft.Tab(
                    text=f"🟢 1 Componente ({len(component_data['wo_1_component'])})",
                    content=create_component_category_table(
                        component_data['wo_1_component'], 
                        "Work Orders con 1 Componente",
                        "🟢",
                        COLORS['success']
                    )
                ),
                ft.Tab(
                    text=f"🟡 2 Componentes ({len(component_data['wo_2_components'])})",
                    content=create_component_category_table(
                        component_data['wo_2_components'], 
                        "Work Orders con 2 Componentes",
                        "🟡", 
                        COLORS['warning']
                    )
                ),
                ft.Tab(
                    text=f"🔵 3-5 Componentes ({len(component_data['wo_3_to_5_components'])})",
                    content=create_component_category_table(
                        component_data['wo_3_to_5_components'], 
                        "Work Orders con 3-5 Componentes",
                        "🔵",
                        COLORS['accent']
                    )
                ),
                ft.Tab(
                    text=f"🔴 +5 Componentes ({len(component_data['wo_more_than_5'])})",
                    content=create_component_category_table(
                        component_data['wo_more_than_5'], 
                        "Work Orders con más de 5 Componentes",
                        "🔴",
                        COLORS['error']
                    )
                )
        ...
        component_analysis_container.current.content.controls.append(component_tabs)
        component_analysis_container.current.visible = True
...
# Crear controles
    ...
    # NUEVO: Contenedor de inicialización
    initialization_container = ft.Container(
        ref=init_container,
        visible=True,
        content=ft.Column([
            ft.Text("🏭 Work Orders Clear Analysis", 
                   size=24, weight=ft.FontWeight.BOLD, color=COLORS['text_primary']),
            ft.Text("Haz clic en 'Cargar Datos' para comenzar el análisis", 
                   size=16, color=COLORS['text_secondary']),
            ft.ElevatedButton(
                "📊 Cargar Datos WO Clear",
                on_click=initialize_wo_clear_tab,
                bgcolor=COLORS['accent'],
                color=COLORS['text_primary'],
            ...
            ft.Text("💡 Asegúrate de haber ejecutado el Dashboard Principal primero", 
    ...
    # NUEVO: Contenedor de análisis por componentes
        ft.Container(
            ref=component_analysis_container,
            visible=False,
            content=ft.Column([]),
            padding=ft.padding.all(20),
            bgcolor=COLORS['primary'],
