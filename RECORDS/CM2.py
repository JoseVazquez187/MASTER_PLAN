...
class ExecutiveDashboard:
    ...
    def create_kpi_card(self, title, value, color, subtitle, drill_type=None):
        """Crea tarjeta KPI moderna con glassmorphism y drill-down"""
        ...
        # Agregar flecha si tiene drill-down
        if drill_type:
            card_content.controls.append(
                ft.Container(
                    content=ft.Icon(ft.Icons.KEYBOARD_ARROW_DOWN, color="#bdbdbd", size=20),
        ...
        # Agregar evento de click si tiene drill-down
        if drill_type:
            container.on_click = lambda e: self.handle_kpi_drilldown(drill_type)
            container.ink = True
